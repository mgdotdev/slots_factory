...
from slots_factory.tools._slots_factory_tools import (
    _slots_factory_hash,
...
def slots_from_dict(attrs={}, __name__="SlotsObject", **kwargs):
    ...
    instance
    :param __name__: name of returned object
    ...
    type_ = fast_slots.__dict__.get(__name__)
    if not type_:
        fast_slots.__dict__[__name__] = type_factory(attrs.keys(), __name__, **kwargs)
    return fast_slots(__name__, **attrs)


def type_factory(args, __name__="Slots_Object", _bases=(), _metaclass=type, **kwargs):
    """function that returns a new Python type w/ __slots__, and other dunder
    methods if specified.

    :param __name__: name of the new type
    :type __name__: str
...
def slots_factory(__name__="SlotsObject", **kwargs):
    """Factory function for creating python objects with __slots__.

    :param __name__: type name to tag to type definition, defaults to "SlotsObject"
    :type __name__: str, optional
    ...
    :rtype: SlotsObject
    """
    id_ = _slots_factory_hash(__name__, kwargs)
    type_ = slots_factory.__dict__.get(id_)
    if not type_:
        type_ = type_factory(kwargs.keys(), __name__, order=kwargs.keys())
...
def fast_slots(__name__="SlotsObject", **kwargs):
    ...
    if not type_:
        type_ = type_factory(kwargs.keys(), __name__, order=kwargs.keys())
        fast_slots.__dict__[__name__] = type_
    try:
        instance = type_()
        _slots_factory_setattrs_slim(instance, kwargs, True)
        return instance
    except AttributeError:
        del fast_slots.__dict__[__name__]
        return fast_slots(__name__, **kwargs)
